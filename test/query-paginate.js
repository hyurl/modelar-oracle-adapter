...
describe("Query.prototype.paginate()", function () {
    it("should get paginated users that suit the given condition", function (done) {
        ...
        co(function* () {
            ...
            assert.equal(query.sql, 'select * from (select * from "users" where "id" in (' + Array(20).fill("?").join(", ") + ') order by "id") where rownum <= 10');
            ...
            assert.equal(query2.sql, 'select * from (select tmp.*, rownum "_rn" from (select * from "users" where "id" in (' + Array(20).fill("?").join(", ") + ') order by "id") tmp where rownum <= 15) where "_rn" > 10');

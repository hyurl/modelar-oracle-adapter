        ...
        co(function* () {
            ...
            return _model.use(db).whereIn("id", ids)
                .getMany().then(function (res) {
                    assert.equal(_model.sql, 'select * from (select * from "users" where "id" in (' + Array(10).fill("?").join(", ") + ') order by "id" asc) where rownum <= 10');
            ...
            return _model.use(db).whereIn("id", ids)
                .getMany({
                    ...
                    assert.equal(_model.sql, 'select * from (select tmp.*, rownum "_rn" from (select * from "users" where "id" in (' + Array(10).fill("?").join(", ") + ') and (("name" like ?) or ("email" like ?)) order by "id" desc) tmp where rownum <= 10) where "_rn" > 5');

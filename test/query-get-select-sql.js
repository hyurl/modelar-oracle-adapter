...
describe("Query.prototype.getSelectSQL()", function () {
    ...
    it("should generate an SQL with where clause and limit clause", function () {
        ...
        assert.equal(query.getSelectSQL(), 'select * from (select "name", "email" from "users" where "id" = ?) where rownum <= 10');
    });

    it("should generate an SQL with where clause, limit and offset clause", function () {
        query.limit(10, 6);

        assert.equal(query.getSelectSQL(), 'select * from (select tmp.*, rownum "_rn" from (select "name", "email" from "users" where "id" = ?) tmp where rownum <= 16) where "_rn" > 6');
    });

    it("should generate an SQL with where clause, limit and offset clause, and order by clause", function () {
        query.orderBy("name");

        assert.equal(query.getSelectSQL(), 'select * from (select tmp.*, rownum "_rn" from (select "name", "email" from "users" where "id" = ? order by "name") tmp where rownum <= 16) where "_rn" > 6');
    });

    it("should generate an SQL with where clause, limit and offset clause, and order by asc clause", function () {
        query.orderBy("id", "asc");

        assert.equal(query.getSelectSQL(), 'select * from (select tmp.*, rownum "_rn" from (select "name", "email" from "users" where "id" = ? order by "name", "id" asc) tmp where rownum <= 16) where "_rn" > 6');
    });

    it("should generate an SQL with where clause, limit and offset clause, and order by rand() clause", function () {
        query.random();

        assert.equal(query.getSelectSQL(), 'select * from (select tmp.*, rownum "_rn" from (select "name", "email" from "users" where "id" = ? order by dbms_random.value()) tmp where rownum <= 16) where "_rn" > 6');
    });
});